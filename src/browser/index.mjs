...
export default class Browser {
  ...
  async configVPN(extensionPath, extensionId) {
    await this.initPage([
      `--disable-extensions-except=${extensionPath}`,
      `--load-extension=${extensionPath}`
    ])
    const pageUrl = `chrome-extension://${extensionId}/popup.html`; // Path to specific part of the extensio
    await this.page.goto(pageUrl);
    await this.page.waitForSelector('#root div.application');
    await click({
      component: this.page,
      selector: '#root div.application ul li'
    })
    await this.page.waitForFunction(() => !document.querySelector('#root .application ul li .spinner-border'));
    // check
    do {
      const messages = await this.page.$$('#root .application .modal .error');
      if (messages.length) {
        await click({ component: this.page, selector: '#root .application .error .close' });
      } else {
        break;
      }
      await click({
