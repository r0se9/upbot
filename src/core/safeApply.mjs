...
async function getJobs(agent) {
  ...
  const jobs = result.map((el) => {
    const publishedOn = moment(el.renewedOn ? el.renewedOn : el.publishedOn).tz(
      "UTC"
    );
    const result = {
      uid: el.uid,
...
const BANNED_COUNTRIES = ["India", "Pakistan", "South Korea"];
function filterJobs(jobs, exclude) {
  return _.filter(jobs, (job) => {
    let score = 0;
    if (exclude.includes(job.uid)) return false;
    if (job.category.category.prefLabel === "Web, Mobile & Software Dev")
      return false;
    if (
      job.client.location.country &&
      BANNED_COUNTRIES.includes(job.client.location.country)
    )
      score--;
    if (job.isFixed) {
      ...
      else if (job.budget.min + job.budget.max > 2 * process.env.HOURLY_LIMIT)
        score++;
    }
...
async function apply(agent, job, gpt, MODE, USEGPT) {
  ...
  const [coverLetter, { engagementDuration, questions }] = await Promise.all([
    (async () => {
      ...
      if (USEGPT) {
        ...
        console.log(
          chalk.green(`GPT is created in ${moment().diff(start) / 1000}s`)
        );
      } else {
  ...
  const result = await agent.applyJob(job.uid, {
    ...
    questions: questions.map((el) => ({
      ...el,
      answer: gpt.getAnswer(el.question),
    })),
  ...
  console.log(
    chalk.green(`Application is finshed in ${moment().diff(start) / 1000}s`)
  );
  return result;
}
async function followUp(database, agent, email, job, result, MODE, USER) {
  if (result.status === "success") {
    ...
    console.log(
      chalk.green(`Successfully Applied in ${now.diff(postedAt) / 1000} s`)
    );
    ...
    if (
      ...
      result.data &&
      result.data.error &&
      result.data.error.message_key ===
        "jpb_Opening_DefaultForbidden_ErrorMessage"
    ) {
      ...
      result.data &&
      result.data.error &&
      result.data.error.message_key === "jpb_ThisJobIsNoLongerAvailable"
    ) {
      console.log(chalk.red(">>>> Job is no longer Available."));
      await database.create("applied", {
        uid: job.uid,
        status: "no longer avaialble",
      });
    } else if (
      result &&
      result.data &&
      result.data.error &&
      result.data.error.message_key === "jpb_codeExtended_VJF_JA_2"
    ) {
      console.log(chalk.red(">>>> Job is no longer Available."));
      await database.create("applied", {
        uid: job.uid,
      ...
      result.data &&
      result.data.error &&
      result.data.error.message_key === "jpb_codeExtended_VJF_CONN_1"
    ) {
      ...
      result.data.error &&
      result.data.error.message_key ===
        "jpb_Opening_DefaultServerError_ErrorMessage"
    ) {
      console.log(
        chalk.red("Server is temporarily down. Try again in a while.")
      );
      await database.delete("accounts", { email: email });
    } else {
...
async function main(gpt, database, USER, MODE, DEBUG, USEGPT) {
  while (true) {
    try {
      let accounts = await database.get(
        "accounts",
        { status: "active", botName: process.env.BOT, name: USER },
        { sort: { createdAt: -1 } }
      );
      if (accounts.length === 0) {
        ...
        do {
          ...
          const saved = await database.get("applied", {
            uid: { $in: jobs.map((el) => el.uid) },
          });
